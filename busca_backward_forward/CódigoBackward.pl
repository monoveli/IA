...
% Argumentos: nó inicial I, nó final F, e o caminho solução (uma lista) entre os 2 
% O primeiro elemento da nossa lista-solução tem que ser o nó inicial, e o último, o nó final 
% Como é BACKWARD, será construída de trás para frente 

caminho_cegoBPBackward(I,F,Cam):- 
    caminho_b(I,[F],Cam).

% Condição de parada %
% Quando o primeiro elemento da lista for o I, já que estamos a percorrendo de trás para frente 
...
% e chamamos o caminho_b recursivamente para repetição do processo, até que a condição de parada seja atingida.
% Note que é acrescentado o elemento Est na lista, e a condição de parada se atinge quando Est = I
...
% caminho_cegoBPBackward(sp,mg,Cam)
% caminho_cegoBPBackward(1,15,Cam) 
